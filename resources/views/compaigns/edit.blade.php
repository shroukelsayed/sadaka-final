...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('compaigns.update', $compaign->id) }}" method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group @if($errors->has('title')) has-error @endif">
                       <label for="title-field">Title</label>
                    <input type="text" id="title-field" name="title" class="form-control" value="{{ $compaign->title }}"/>
                       @if($errors->has("title"))
                    ...
                    <textarea class="form-control" id="location-field" rows="3" name="location">{{ $compaign->location }}</textarea>
                    ...
                    <input  type="text" id="startdate-field" name="startdate" class="form-control" value="{{ $compaign->startdate }}"/>
                    ...
                    <input  type="text" id="enddate-field" name="enddate" class="form-control" value="{{ $compaign->enddate }}"/>
                    ...
                    <input type="text" id="budget-field" name="budget" class="form-control" value="{{ $compaign->budget }}"/>
                    ...
                    <div class="form-group">
                      ...
                      <select name="governrate" id="modify_modal_level">
                          @foreach ($governrates as $key => $value)
                              <option value="{{ $key+1 }}">{{ $compaign->governorate->name }}</option>
                          @endforeach
                      </select>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="city" id="modify_modal_level">
                          @foreach ($city as $key => $value)
                              <option value="{{ $key+1 }}">{{ $compaign->city->name }}</option>
                          @endforeach
                      </select>
                    </div>
                    <div class="form-group @if($errors->has('description')) has-error @endif">
                       <label for="description-field">Description</label>
                    <textarea class="form-control" id="description-field" rows="3" name="description">{{ $compaign->description }}</textarea>
